// analyze_program() takes in a start and end address, and outputs an
...
// 2.3.4.1.
//
// To be more specific, analyze_program() computes a weighted sum
...
#ifndef ANALYZE_PROGRAM_H
#define ANALYZE_PROGRAM_H
...
void analyze_program(int start, int end, MMM *mmm);

#ifdef ANALYZE_PROGRAM_IMPLEMENTATION
...
void tracecycle(int cyclestart, int cycleend, int cycleid, int Start, int End, int *parent, bool *S, edge **N, edge *E, MMM *mmm) {
  byte C, F; int cycle_count;
  if (cycleend != End) {
    // Count the edge causing the cycle itself into the sum.
    C = get_C(mmm->mix.mem[cycleend]);
    F = get_F(mmm->mix.mem[cycleend]);
    cycle_count = get_exact_cycle_count(C, F);
    E[cycleid].weight = cycle_count;
  }
  ...
  do {
    ...
    if (s != Start && !(s == End && d == Start)) {
      C = get_C(mmm->mix.mem[s]);
      F = get_F(mmm->mix.mem[s]);
      cycle_count = get_exact_cycle_count(C, F);
      if (S[j])
        E[cycleid].weight += cycle_count;
      else
...
void analyze_program(int start, int end, MMM *mmm) {
  ...
  Arena tmp_arena = *mmm->tmp_arena;

  // For building up the final output string
  static StrBuf sb = sb_empty_init;
  if (sb_is_empty(sb))
    sb = sb_alloc(1024, arena);
  sb_clear(&sb);

  // Each instruction from start to end is a node; there are
  // also special Start and End nodes added at the boundaries.
  int numnodes = end - start + 3;
  int Start = start-1, End = end+1;

  // The subroutine progressively builds an oriented subtree of the
  // directed graph of execution flow from start to end.
  // parent[j]=-1 means j has no parent;
  int *parent = arena_alloc(numnodes * sizeof(int), &tmp_arena);
  ...
  bool *S = arena_alloc(numnodes * sizeof(bool), &tmp_arena);
  // Make S[Start] point to the first element.
  S -= Start;

  // E lets us access edges by their id.
  edge *E = arena_alloc(numnodes*2*sizeof(edge), &tmp_arena);
  ...
  edge **N = arena_alloc(numnodes*2*sizeof(edge *), &tmp_arena);
  ...
  for (int i = Start; i <= End; i++) {
    if (i == Start) {     // Start has exactly one edge: to start
      C = JMP; F = 0; A = start;
    }
      ...
      if (mmm->line_infos[i].is_con_or_alf)
        continue;
    ...
    else if (i != End && (A < start || A > end)) A = End;
...
nextedge:
  ...
  for (int i = 0; i < curid; i++) {
    ...
    sb_appendf(&sb, "E%d: %s\t" sv_fmt, arena,
               i,
               ee->branch ? "# TIMES TAKEN" : "# TIMES UNTAKEN",
               sv_arg(mmm->line_infos[ee->s].mixal_line));
  }
  sb_append_cstr(&sb, "Total time = ", arena);
  for (int i = 0; i < curid; i++) {
    edge *ee = E+i;
    if (!ee->iscycle || ee->weight == 0) continue;
    if (i == curid-1)
      sb_appendf(&sb, "+ %d", arena, ee->weight);
    else
      sb_appendf(&sb, "+ %d*E%d ", arena, ee->weight, i);
  }
  sb_append_cstr(&sb, "\n", arena);
  my_print(sb.s);
}

#endif // ANALYZE_PROGRAM_IMPLEMENTATION
#endif // ANALYZE_PROGRAM_H